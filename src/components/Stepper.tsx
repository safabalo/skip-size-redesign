...
const Stepper: React.FC<StepperProps> = ({ steps, isDarkMode, variant = 'default' }) => {
  ...
  return (
    <div className={`flex justify-center mb-8 ${isCompact ? 'mb-6' : 'mb-8'}`}>
      <div className="flex items-center space-x-1 overflow-x-auto pb-2 md:pb-0 max-w-full">
        {stepperSteps.map((step, index) => (
          <React.Fragment key={step.id}>
            <div className={`flex items-center space-x-1 md:space-x-2 px-2 md:px-3 py-2 rounded-full text-xs md:text-sm whitespace-nowrap transition-all duration-200 ${
              step.active 
                ? isDarkMode 
            ...
            } ${isCompact ? 'px-2 py-1' : 'px-2 md:px-3 py-2'}`}>
            ...
            {index < stepperSteps.length - 1 && (
              <ArrowRight className={`flex-shrink-0 ${isCompact ? 'w-3 h-3' : 'w-3 h-3 md:w-4 md:h-4'} ${
